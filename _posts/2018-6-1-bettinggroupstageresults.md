...
So far from revolutionary, but seeing the predictions turn a small profit above the rake is neat as our model bet on *all* games, rather than just the ones it had confidence in. One would hope that this result could be improved upon by actually using the information available leading up to the world cup, such as squad selection, fitness rumors, and injury abscences, to improve the result (however, beating the betting markets tends to be trickier than people expect, even with additional information).
